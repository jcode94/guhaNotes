// Copied from Arup Guha for practice
// Linked List example(chaining for strings)
...
// Struct used to form a linked list of integeres.
struct ll
{
...
int main()
{
    ...
    // Read in all of the word from a file into the table.
...
// Post-condition: word will beinserted into the table h.
void insertTable(struct htable *h, char word[])
{
    int hashval;
...
// Pre-condition: h points to a valid hash table.
// Post-conditionn: 1 will be returned iff word is stored in the table pointed
//                  to by h. Otherwise, 0 is returned.
int searchTable(struct htable *h, char word[])
{
    int hashval;
    hashval = hashvalue(word);
...
// Pre-condition: h opints to a valid hash table.
...
// Pre-condition: front points to the head of a linkned list structure.
// Post-conditoin: each value in the list will be printed, in order.
void print(struct ll *front)
{
    while(front != NULL)
    {
...
// Pre-condition: front points tot he head of a linked list structure.
...
//                 removed from the list. A pointer to the front of
//                 the list will be returned. If no such value is stored,
...
struct ll* delete(struct ll *front, char word[])
{
    ...
    if(temp != NULL)
    {
        ...
        // Otherwise, loop until you find the node to delete and to do so.
